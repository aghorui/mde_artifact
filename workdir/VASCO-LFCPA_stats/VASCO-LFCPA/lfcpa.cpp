        ...
        if (argCallee != nullptr) { // do this only for functions with return argument
            for (auto pin : a3) {
                if (compareOperands(GET_PTSET_KEY(pin), argCallee)) { // x=call fun() and return x' in
                    ...
                    retForwardOutflow = retForwardOutflow.update_pointees(GET_PTSET_KEY(pin), GET_VALUE(pin)); // modAR::MDE
                }
            }
            if (debugFlag) {
...
int main(int argc, char *argv[]) {
    ...
    llvm::outs() << "@@@@ EQUALITY COMPARISONS: " << STAT_equality_comparisons << "\n";
    llvm::outs() << "@@@@ ENUMERATION LIVENESS: " << STAT_enumerations_liveness << "\n";
    llvm::outs() << "@@@@ ENUMERATION POINTSTO: " << STAT_enumerations_pointsto << "\n";
    llvm::outs() << "@@@@ CONSTRUCTION LIVENESS: " << STAT_enumerations_liveness << "\n";
    llvm::outs() << "@@@@ CONSTRUCTION POINTSTO: " << STAT_enumerations_pointsto << "\n";
    llvm::outs() << "@@@@ ASSIGNMENT LIVENESS: " << STAT_assignments_liveness << "\n";
    llvm::outs() << "@@@@ ASSIGNMENT POINTSTO: " << STAT_assignments_pointsto << "\n";
    llvm::outs() << livenessMDE.dump_perf() << "\n";
    llvm::outs() << pointsToMDE.dump_perf() << "\n";
