...
#if defined(NAIVE_MODE)
#include "CommonNaive.h"
#elif defined(MDE_MODE)
#include "Common.h"
#elif defined(ZDD_MODE)
#include "CommonZDD.h"
#elif defined(SPARSEBV_MODE)
#include "CommonSparseBV.h"
#elif defined(SINGLE_LEVEL_MODE)
#include "CommonSingleLevel.h"
#endif
    ...
    if (SLIM) {
            ...
            if (modeOfExec == FSFP || modeOfExec == FSSP) {
                for (int icontext = 0; icontext <= context_label_counter; icontext++) {
                    ...
                    slim::Function *slimFunc = optIR->getSlimFromLlvm(currFun);

                    llvm::outs() << "\n Function : " << currFun->getName() << "\n";
                    for (auto block = slimFunc->getBlocksBegin();
                        block != slimFunc->getBlocksEnd(); ++block) {
                        for (auto inst_cursor = (*block)->getInstructionsBegin();
                             inst_cursor != (*block)->getInstructionsEnd(); inst_cursor++) {
                            llvm::outs() << "[Context : " << icontext << "]  Instruction : [["
                                             << (*inst_cursor)->getInstructionId() << "]] ";
                            (*inst_cursor)->printInstruction();
                            llvm::outs() << "PIN value : ";
                            printDataFlowValuesForward(
                                getForwardComponentAtInOfThisInstruction(*inst_cursor, icontext));
                            llvm::outs() << "\nPOUT value : ";
                            printDataFlowValuesForward(
                                getForwardComponentAtOutOfThisInstruction(*inst_cursor, icontext));
                            llvm::outs() << "\nLOUT value : ";
                            printDataFlowValuesBackward(
                                getBackwardComponentAtOutOfThisInstruction(*inst_cursor, icontext));
                            llvm::outs() << "\nLIN value : ";
                            printDataFlowValuesBackward(
                                getBackwardComponentAtInOfThisInstruction(*inst_cursor, icontext));
                            llvm::outs() << "\n\n";
                        }
                    }
