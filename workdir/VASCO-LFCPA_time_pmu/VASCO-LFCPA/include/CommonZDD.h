#ifndef LFCPA_COMMON_ZDD_H
#define LFCPA_COMMON_ZDD_H
// #include "Analysis.h"
#include "slim/IR.h"
#include <utility>
...
struct LFLivenessSet {
    using container_type = ZDD;
    using key_type = SLIMOperand *;
    using value_type = SLIMOperand *;
    using iterator = ZDDSLIMOperandIterator;
    ...
    static LFLivenessSet create_from_single(key_type p) {
        ...
        return LFLivenessSet(ZDD(id_mapper.get_id(p)));
    }
    ...
    LFLivenessSet get_purely_global() const {
        ...
        for (SLIMOperand *i : ZDDSLIMOperandIterator(elems)) {
    ...
    LFLivenessSet insert_single(key_type p) {
        STAT_operation_count++;
        ZDD result = elems.insert_single(id_mapper.get_id(p));
    ...
    LFLivenessSet remove_single(key_type p) {
        STAT_operation_count++;
        ZDD result = elems.remove_single(id_mapper.get_id(p));
        return std::move(result);
    }
    ...
    ZDDSLIMOperandIterator begin() const {
        return ZDDSLIMOperandIterator(elems).begin();
    }

    ZDDSLIMOperandIterator end() const {
        return ZDDSLIMOperandIterator(elems).end();
    }
...
struct LFPointsToSet {
    using container_type = std::map<SLIMOperand *, ZDD>;
    using key_type = SLIMOperand *;
